...
- [ ] `TASK-015` Capas unificadas (vídeo+imagen+texto en un z-stack) — épico en curso
  ...
  - **PIVOT a carriles "media" mixtos (2026-06-21)**: por decisión del usuario, se abandona el modelo de capas TIPADAS (vídeo/imagen/texto por separado) y un carril pasa a contener elementos MIXTOS (vídeo+imagen+texto) en secuencia (sin solape por carril). Orden de carriles = z. Audio y subtítulos aparte. Esto supera/reemplaza las Fases 1-3 tipadas.
    ...
    - **Pendiente**: Media M4 (pulido). VERIFICACIÓN VISUAL pendiente por el usuario: z en vivo (texto/imagen/vídeo entremezclados), recorte de imagen/vídeo con el nuevo render HTML, selección de elementos que queden detrás. Limpiar stubs muertos `videoLayers/imageLayers/textLayers` y wrappers deprecados del store (`addVideoTrack/addImageLayer/addTextLayer/moveClipToTrack`).
