...
## In Progress
- [x] `TASK-015` Capas unificadas (vídeo+imagen+texto en un z-stack) — **COMPLETO** (modelo "media" mixto, 2026-06-21)
  ...
  - **PIVOT a carriles "media" mixtos (2026-06-21)**: por decisión del usuario, se abandona el modelo de capas TIPADAS (vídeo/imagen/texto por separado) y un carril pasa a contener elementos MIXTOS (vídeo+imagen+texto) en secuencia (sin solape por carril). Orden de carriles = z. Audio y subtítulos aparte. Esto supera/reemplaza las Fases 1-3 tipadas.
    - **Media M1 (modelo v4 + store + export) — HECHA (2026-06-21)**: esquema `MediaLayer{items: MediaElement[]}` con `MediaElement` = unión por `kind` (video/image/text); `migrateMedia` v3→v4 (encadenada tras `migrateProject`/`migrateLayers`); store reescrito sobre `items` con no-solape MIXTO por carril (`findElement`/`mediaLayerFor`/`addMediaLayer`/`moveElementToLayer` cualquier-kind→cualquier-carril); `filterGraph` compone iterando `layer.items` por orden temporal y `kind` (equivalente a proyectos migrados, etiquetas únicas); lectores UI adaptados (preview/timeline mínimos, fix anti-bucle Zustand conservado); e2e ffmpeg de capa media mixta `[video, image, text]`. 35 shared + 107 client + 152 server verdes, typecheck limpio en los 3 paquetes. Plan: `docs/superpowers/plans/2026-06-21-capas-media-M1-modelo.md`. Commits: `7f537df`, `7b0df04`, `3d7a0d9`, `c84ccf2`, `0ba8260`, `cd9d385`.
    - **Media M2 (timeline unificado) — HECHA (2026-06-21)**: Timeline sobre carriles "media" genéricos (sin distinguir vídeo/imagen/texto). Cada carril = una `MediaLayer` y muestra sus bloques mixtos; título genérico "Capa N" (orden visual). Botón único "+ Capa" y `+` por carril (inserta capa encima). Arrastre vertical de un elemento estilo Adobe con 3 bandas (tercio superior = insertar capa ENCIMA, central = mover DENTRO del carril, inferior = insertar DEBAJO) → inserción entre CUALQUIER par de carriles, con línea-indicador "+ nueva capa" en el hueco (no solo en los extremos). Reordenado de capas arrastrando la cabecera; cualquier carril se puede quitar (`removeLayer` garantiza ≥1). `moveElementToLayer` ya no restringe por tipo (cualquier kind a cualquier capa). Store: `addMediaLayer(atIndex?)` inserta en posición. 35 shared + 131 client + 152 server verdes, typecheck limpio. Plan integrado en M1; commit `1b294e2`.
    - **Media M3 (preview compositado por z) — HECHA (2026-06-21)**: opción A — todo lo visual (vídeo/imagen/texto) se pinta en HTML/CSS apilado por `zIndex` = índice de capa (orden de capas = z), y Konva queda como capa transparente por encima (zIndex 50) solo para asas/transformador/recorte/guías/subtítulos. Helpers puros `overlayCss.ts` (`imageOverlayStyle`/`imageInnerStyle`/`textOverlayStyle`) con paridad de geometría con Konva (centro como origen, rotación, crop→overflow+img expandida, stroke/shadow→CSS). `LayerOverlayHtml` por capa se suscribe al playhead. `ImageNode`/`TextNode` de Konva pasan a `opacity={0}` (conservan hit-area y medida para el Transformer). Ahora un texto puede verse DETRÁS de un vídeo y una imagen ENTRE vídeos en la vista previa en vivo, igual que en el export. Plan: `docs/superpowers/plans/2026-06-21-capas-media-M3-preview.md`. Commits: `73772db` (plan), helpers + preview.
    - **FIX z en vivo (2026-06-21)**: el `<video>` compositado por GPU se "escapaba" por encima de texto/imagen de capas superiores. Solucionado con envoltura por capa en orden de DOM (= z) + `isolation: isolate` en el marco del lienzo. También se arregló un bug grave: el stub muerto `videoLayers()` devolvía `[]` y dejaba el vídeo base sin `src` (negro) y rompía export/medios/subtítulos/toolrail → reemplazado por el selector real `videoTracks()`. Commits `fc3f111`, `e664a66`. Verificado por el usuario: "está perfecto ahora".
    - **Media M4 (pulido) — HECHA (2026-06-21)**: eliminados los wrappers deprecados del store (`addVideoTrack`/`addImageLayer`/`addTextLayer`/`moveClipToTrack`/`reorderVideoTrack`/`removeVideoTrack`) y los stubs muertos `videoLayers/imageLayers/textLayers` de shared. 35 shared + 125 client + 152 server verdes, typecheck limpio. Commit `073a8fc`.
    - **Verificación pendiente menor**: recorte de imagen/vídeo y reproducción con varias capas (el usuario confirmó el z; conviene una pasada de recorte/play cuando pueda).
