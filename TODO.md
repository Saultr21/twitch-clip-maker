...
- [ ] `TASK-012` — Multipista: varias líneas de medios (vídeo/imagen superpuestos, PiP)
  ...
  - **Pendiente**: Fase 5 (control de **opacidad por clip** en el panel de Propiedades, cableado a `updateVideoClip({ opacity })`). Marcadores `TODO(fase2)` en `projectStore.ts` (ripple/split por pista, y "Dividir" opera solo sobre la base).
  - Priority: high
