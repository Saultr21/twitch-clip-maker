...
- [ ] `TASK-012` — Multipista: varias líneas de medios (vídeo/imagen superpuestos, PiP)
  ...
  - **Pendiente**: Fase 3 (preview: N `<video>` + motor multipista), Fase 4 (timeline multipista: carriles, añadir/quitar, arrastre entre pistas — `addVideoTrack`/`removeVideoTrack`/`moveClipToTrack`), Fase 5 (opacidad por capa en UI). Marcadores `TODO(fase2)` en `projectStore.ts` (ripple/split por pista — pendientes para multipista real). Cada fase tendrá su propio plan.
  - Priority: high
