...
import { migrateMedia, migrateLayers, migrateProject, projectSchema } from "@clipforge/shared";
...
function tryRead(file: string): Project | null {
  try {
    const raw = migrateMedia(migrateLayers(migrateProject(JSON.parse(fs.readFileSync(file, "utf8")))));
